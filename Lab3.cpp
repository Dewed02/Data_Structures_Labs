...
string  findBinaryNotation(int num)
{
    ...
    if (num != 0)
    {
        //Recurisevly call function with next quotient
        // Return remainder of all quotient in resverse order
...
{ 
    ...
    {
        // True is all characters match or all but one for odd length string
        return true;
    }
    else if (sub.front() == sub.back())
    {
...
void arrRev(char(&arr)[26], int low, int high)
{
    if (low != (high/2))
    { 
        // Temp variable to hold value at end of list to be changed
        char temp = arr[(high - low)];
        // Assign value at the end of list the value at the begining 
        arr[(high - low)] = arr[low];
        // Assign value at the being of list the temp value
        arr[low] = temp;
        low = low + 1;
        return arrRev(arr, low, high);
    }
}
